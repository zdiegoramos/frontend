...
describe("invoiceRouter", () => {
  ...
  describe("create", () => {
    ...
    it("throws when DB returns empty result", async () => {
      ...
      await expect(call(appRouter.invoice.create, validInput)).rejects.toThrow(
  ...
  describe("update", () => {
    it("updates and returns the invoice by nanoId", async () => {
      ...
      mockReturning.mockResolvedValue([
        { ...mockInvoice, merchant: "Updated Corp" },
      ]);
      ...
      expect(result).toEqual({
        invoice: expect.objectContaining({ merchant: "Updated Corp" }),
      });
